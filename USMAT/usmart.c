...
void usmart_reset_runtime(void)
{
    ...
    __HAL_TIM_SET_AUTORELOAD(&TIM6_Handler,0XFFFF);     //将重装载值设置到最大
    __HAL_TIM_SET_COUNTER(&TIM6_Handler,0);             //清空定时器的CNT
...
{
	if(__HAL_TIM_GET_FLAG(&TIM6_Handler,TIM_FLAG_UPDATE)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=__HAL_TIM_GET_COUNTER(&TIM6_Handler);
...
{ 		    		  			       
    if(__HAL_TIM_GET_IT_SOURCE(&TIM6_Handler,TIM_IT_UPDATE)==SET)//溢出中断
    {
        ...
        // 处理定时器 10 更新中断
			...
			__HAL_TIM_SET_COUNTER(&TIM6_Handler,0);    //清空定时器的CNT
			__HAL_TIM_SET_AUTORELOAD(&TIM6_Handler,100);//恢复原来的设置
...
//    TIM6_Handler.Instance=TIM10;                          //通用定时器4
//    TIM10_Handler.Init.Prescaler=psc;                     //分频
...
//		HAL_NVIC_SetPriority(TIM1_UP_TIM10_IRQn,10,0);    //设置中断优先级，抢占优先级3，子优先级3
//    HAL_NVIC_EnableIRQ(TIM1_UP_TIM10_IRQn);          //开启ITM4中断  
//    HAL_TIM_Base_Start_IT(&TIM10_Handler); //使能定时器4和定时器4中断 	
//   __HAL_RCC_TIM6_CLK_ENABLE();
//     
    TIM6_Handler.Instance=TIM6;                          //通用定时器4
    TIM6_Handler.Init.Prescaler=psc; //216-1                    //分频
    TIM6_Handler.Init.CounterMode=TIM_COUNTERMODE_UP;    //向上计数器
    TIM6_Handler.Init.Period=arr;   //999                      //自动装载值
...
{
#if USMART_ENTIMX_SCAN==1
	Timer6_Init(1000,(u32)sysclk*1/*100*/-1);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
